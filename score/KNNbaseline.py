...
class KNNbase_Unlearning():
    ...
    def unlearning_request(self, history_index, movie_index):
        ...
        self.data_df_droped.to_csv("../data/u-unlearning{}.csv".format(history_index), sep="\t", header=None,
                                   index=False)
    ...
    def recommendation_unlearning(self, forgetting_index, validate_flag):
        ...
        if validate_flag:
            alg, rmse, mae = self.train_unlearning_model_crossvalidate()
            self.RMSE.append(rmse)
            self.MAE.append(mae)
        else:
            alg = self.train_model()
