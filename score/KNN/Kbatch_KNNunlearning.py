    ...
    def unlearning_request(self, history_index: list):
        ...
        if self.shards > 1:
            if self.shuffle:  # shuffled
                self.data_df_droped.to_csv("../../data/shards_{}_shuffled/u-unlearning{}.csv".format(self.shards, history_index[-1]),
                                           sep="\t", header=None, index=False)
            else:  # ordered
                self.data_df_droped.to_csv("../../data/shards_{}_ordered/u-unlearning{}.csv".format(self.shards, history_index[-1]),
                                           sep="\t", header=None, index=False)
        else:
